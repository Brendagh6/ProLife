...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (u.rol === 'super_admin') {
        setAppState('dashboard'); // o 'superadmin'
      } else {
        checkOnboarding(u);
      }
    } else {
 ...
 const handleLogin = (userData) => {
    console.log("Usuario logueado:", userData); // ← para debug
    setUser(userData);
...
if (appState === 'loading') {
    return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#05091a' }}>Cargando...</div>;
  }

  if (appState === 'login') return <LoginPage onLogin={handleLogin} />;

  if (appState === 'onboarding') {
    return <OnboardingForm user={user} onComplete={() => checkOnboarding(user)} />;
  }

  // Si es Super Admin → mostrar panel completo
  if (user?.rol === 'super_admin') {
    return <SuperAdminDashboard user={user} onLogout={handleLogout} t={DARK} />;
  }
  const TABS = [
